...
public final class JavaFileObjects {
    ...
    public static JavaFileObject forSourceString(String fullyQualifiedName, String source) {
        ...
        try {
            return new MemoryJavaFileObject(fullyQualifiedName, source, SOURCE);
        } catch (IOException e) {
            throw new IllegalArgumentException(
                    String.format("Error with (%s).", fullyQualifiedName), e);
        }
    }
    ...
    private static final class StringSourceJavaFileObject extends SimpleJavaFileObject implements FullyQualifiedName {
        final String source;
    ...
    public static Kind deduceKind(URI uri) {
